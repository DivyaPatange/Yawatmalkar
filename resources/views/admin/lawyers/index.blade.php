...
@endsection
@section('page_title', 'Lawyers List')
@section('breadcrumb1', 'Home')
@section('breadcrumb2')
<a href="{{ route('admin.lawyers.index') }}">Lawyers List</a>
@endsection
@section('content')
<!-- <div class="row">
    <div class="col-sm-12">
...
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">
                <h5>Lawyers List</h5>
                <a href="{{ route('admin.lawyers.create') }}"><button type="button" class="btn btn-outline-primary float-right" title="" style="padding:10px 20px">Add New</button></a>
            </div>
            <div class="card-body">
...
var SITEURL = '{{ route('admin.lawyers.index')}}';
function format ( d ) {
...
$('body').on('click', '.switch_1', function () {
    ...
    if(id != ''){
        $.ajax({
            type: "get",
            url: "{{ url('admin/lawyers/status') }}"+'/'+id,
            success: function (data) {
                // alert(data.teacher);
            var oTable = $('#zero_config').dataTable(); 
            oTable.fnDraw(false);
